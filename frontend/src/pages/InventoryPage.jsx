...
import { Search, AlertTriangle, Plus, Minus, SlidersHorizontal, X } from 'lucide-react';
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        width: '100%', maxWidth: 400, padding: '1.75rem',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
          <div>
            ...
            <p style={{ color: '#8a8680', fontSize: '0.82rem', margin: '0.2rem 0 0' }}>
              Stock actual: <strong style={{ color: '#f0ede8' }}>{product.stock}</strong>
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {/* Tipo de movimiento */}
          <div>
            <label style={{ display: 'block', fontSize: '0.78rem', color: '#8a8680', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>
              Tipo de movimiento
            </label>
          ...
          {cantidad && parseInt(cantidad) > 0 && (
            <div style={{ background: '#211f1c', border: '1px solid #2e2b27', borderRadius: 10, padding: '0.75rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                ...
                <span style={{ fontFamily: "'Syne', sans-serif", fontSize: '1.2rem', color: delta > 0 ? '#3ecf8e' : delta < 0 ? '#e85d3a' : '#f0ede8' }}>
                  {nuevo}
                </span>
                {delta !== 0 && (
                  <span style={{ fontSize: '0.78rem', color: delta > 0 ? '#3ecf8e' : '#e85d3a' }}>
                    ({delta > 0 ? '+' : ''}{delta})
                  </span>
                )}
              </div>
...
export default function InventoryPage() {
  ...
  const [stockFilter, setStockFilter] = useState(''); // '' | 'bajo' | 'agotado'
  ...
  return (
    <div style={{ padding: '2rem', color: '#f0ede8', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ marginBottom: '1.75rem' }}>
        <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: '1.8rem', margin: 0 }}>Inventario</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.75rem', marginBottom: '1.75rem' }}>
        {[
          ...
          <div key={label} style={{ background: '#1a1916', border: '1px solid #2e2b27', borderRadius: 14, padding: '1rem' }}>
            <div style={{ fontSize: '0.73rem', color: '#8a8680', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.4rem' }}>{label}</div>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: '1.5rem', color }}>{value}</div>
          </div>
        ))}
      </div>
      ...
      {/* Alertas críticas */}
      {alertas.length > 0 && (
        <div style={{ background: '#2a1a0e', border: '1px solid #f5a62340', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
            <AlertTriangle size={15} color="#f5a623" />
            <span style={{ color: '#f5a623', fontSize: '0.82rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {alertas.length} producto{alertas.length !== 1 ? 's' : ''} con stock bajo o agotado
            </span>
          </div>
            ...
            {alertas.map(p => {
              ...
              return (
                <div key={p._id} onClick={() => setModal(p)} style={{
                  ...
                  <span style={{ color: '#f0ede8', fontSize: '0.82rem' }}>{p.nombre}</span>
                  <span style={{ color: cfg.color, fontWeight: 700, fontSize: '0.82rem' }}>{p.stock}</span>
                </div>
              );
            })}
          </div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        ...
        <select value={catFilter} onChange={e => setCatFilter(e.target.value)}
          style={{ ...inputStyle, width: 'auto', flex: '0 0 155px', cursor: 'pointer' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {products.map((p, i) => {
                const estado = getEstado(p);
                const cfg    = ESTADO_CFG[estado];
                const margen = p.costo && p.costo > 0
                  ? (((p.precio - p.costo) / p.precio) * 100).toFixed(1)
                  : null;

                return (
                  <tr key={p._id}
                    style={{ borderBottom: i < products.length - 1 ? '1px solid #2e2b27' : 'none', transition: 'background 0.12s' }}
                  ...
                  >
                    <td style={{ padding: '0.8rem 1rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.55rem' }}>
                        ...
                        <span style={{ fontWeight: 500, fontSize: '0.9rem' }}>{p.nombre}</span>
                      </div>
                    </td>
                    <td style={{ padding: '0.8rem 1rem' }}>
                      <span style={{ background: '#211f1c', border: '1px solid #2e2b27', borderRadius: 6, padding: '0.18rem 0.5rem', fontSize: '0.76rem', color: '#8a8680' }}>
                        {p.categoria}
                      </span>
                    </td>
                    ...
                    <td style={{ padding: '0.8rem 1rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                        <span style={{ fontFamily: "'Syne', sans-serif", fontSize: '1.1rem', color: cfg.color, fontWeight: 700, minWidth: 28 }}>
                          {p.stock}
                        </span>
                        <div style={{ width: 48, height: 4, background: '#2e2b27', borderRadius: 2, overflow: 'hidden' }}>
                          <div style={{
                            height: '100%', borderRadius: 2, background: cfg.color,
                            width: `${Math.min(100, (p.stock / Math.max(p.stockMinimo * 2, 1)) * 100)}%`,
                    ...
                    <td style={{ padding: '0.8rem 1rem' }}>
                      <span style={{ background: cfg.bg, color: cfg.color, borderRadius: 6, padding: '0.18rem 0.55rem', fontSize: '0.76rem', fontWeight: 600 }}>
                        {cfg.label}
                      </span>
                    </td>
                    <td style={{ padding: '0.8rem 1rem', color: '#8a8680', fontSize: '0.85rem' }}>
                      {p.costo ? S(p.costo) : '—'}
                    </td>
                    <td style={{ padding: '0.8rem 1rem', color: '#f5a623', fontWeight: 600, fontSize: '0.88rem' }}>
                      {S(p.precio)}
                    </td>
                    <td style={{ padding: '0.8rem 1rem' }}>
                      {margen !== null
                    ...
                    <td style={{ padding: '0.8rem 1rem' }}>
                      <div style={{ display: 'flex', gap: '0.35rem' }}>
                        <button onClick={() => setModal(p)} title="Ajustar stock" style={{
                          background: '#211f1c', border: '1px solid #2e2b27', borderRadius: 8,
                          padding: '0.4rem 0.6rem', cursor: 'pointer', color: '#f0ede8',
